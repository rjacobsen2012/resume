    ...
    <div :class="[bgColorSchemeValue, `main-content`]">
        <b-navbar class="top-nav navbar navbar-expand shadow-sm" :type="bgColorValue" :variant="bgColorValue">
            ...
            <b-navbar-nav class="ml-auto notifications pr-2">
                ...
                <b-nav-item class="pointer squared ml-1" v-b-tooltip.hover.bottomleft="'Github Profile'" :href="getResumeUserProperty('github_profile', '#')" target="_blank"><font-awesome-icon :icon="['fab', 'github-alt']"/><span class="icon-visible pl-2">{{ getResumeUserProperty('github_profile', '') }}</span></b-nav-item>
                <b-nav-item class="pointer squared ml-1" title="Email" v-b-tooltip.hover.bottomleft="getResumeUserProperty('email')" :href="`mailto:${getResumeUserProperty('email')}`" target="_blank"><font-awesome-icon icon="envelope"/><span class="icon-visible pl-2">{{ getResumeUserProperty('email', '') }}</span></b-nav-item>
                <b-nav-item :class="[`pointer`, `squared`, `ml-1`, textColorSchemeValue]" v-b-tooltip.hover.bottomleft="`Phone: ${getResumeUserProperty('phone')}`" :href="`tel:${getResumeUserProperty('phone')}`" target="_blank"><font-awesome-icon icon="phone-square"/><span class="icon-visible pl-2">{{ getResumeUserProperty('phone', '') }}</span></b-nav-item>
                <b-nav-item class="pointer squared ml-1" v-b-tooltip.hover.bottomleft="'PDF Resume'" @click="downloadResume" target="_blank"><font-awesome-icon icon="file-pdf"/></b-nav-item>
                <b-nav-item class="pointer squared ml-1" v-b-tooltip.hover.bottomleft="'Word Resume'" @click="downloadResumeWord" target="_blank"><font-awesome-icon icon="file-word"/></b-nav-item>
